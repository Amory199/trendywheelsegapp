...
10. **Consult the knowledge graph before broad codebase exploration.** Skim [graphify-out/GRAPH_REPORT.md](./graphify-out/GRAPH_REPORT.md) for God Nodes + Surprising Connections, or run `/graphify "<question>"` (inside Claude Code) / `graphify query "<question>"` (shell). Regenerate code structure offline with `graphify update .` from repo root after a structural rename or new package. Don't grep 459 source files when the graph already has the answer.
11. **Before opening a PR, run through [CHECKLISTS/FEATURE.md](./CHECKLISTS/FEATURE.md).** Product / correctness gate — types, contract, UX, data, tests, docs. CI rejects PRs missing the completion marker (see `.github/workflows/checklist-gate.yml`).
12. **If the change touches auth, secrets, file upload, role enforcement, or any external integration: also run [CHECKLISTS/SECURITY.md](./CHECKLISTS/SECURITY.md).** This is where past incidents (INC-001 keystore, INC-018 mass-assignment, INC-019 localStorage tokens) would have been caught at PR time.
13. **If the change touches the DB, queues, cache, websocket, or a list endpoint: also run [CHECKLISTS/SCALE.md](./CHECKLISTS/SCALE.md).** Built for the millions-of-users target — catching N+1 / unbounded list / missing index issues at PR time is 10× cheaper than under load. See [AUDIT_FINDINGS.md](./AUDIT_FINDINGS.md) for the canonical list of what surfaces under scale.
