          ...
          <tbody className="tw-stagger">
            {leads.map((l) => (
              <tr key={l.id} style={{ borderTop: "1px solid #ECECF1" }}>
                ...
                <td style={td}>
                  <span
                    style={{
                      ...
                      padding: "3px 10px",
                      borderRadius: 999,
                      background: `${STATUS_COLOR[l.status] ?? "#888"}22`,
                      color: STATUS_COLOR[l.status] ?? "#666",
                      fontSize: 11,
                      fontWeight: 700,
                      textTransform: "uppercase",
                    }}
                  >
                    <span
                      style={{
                        ...
                        height: 6,
                        borderRadius: 3,
                        background: STATUS_COLOR[l.status] ?? "#888",
                      }}
                    />
                    {l.status}
                  </span>
                ...
                <td style={td}>
                  {l.owner?.name ?? <span style={{ color: "#9E9DAE" }}>Unassigned</span>}
                </td>
                <td style={{ ...td, textTransform: "capitalize" }}>
                  {l.source.replace(/[-_]/g, " ")}
                </td>
                <td style={{ ...td, textAlign: "right", fontWeight: 700 }}>
                  EGP {Math.round(Number(l.estimatedValue)).toLocaleString()}
                </td>
                <td style={{ ...td, fontSize: 11, color: "#6B6A85" }}>
            ...
            {leads.length === 0 ? (
              <tr>
                <td
                  colSpan={6}
                  style={{ ...td, textAlign: "center", color: "#6B6A85", padding: 32 }}
                >
                  No leads yet.
                </td>
              </tr>
