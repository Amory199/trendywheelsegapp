...
export default function RentDetailPage(): JSX.Element {
  ...
  // Guests (shared-link visitors) have no token — don't fire an authed loyalty
  // query for them; it 401s and pollutes the error log with benign noise.
  const isSignedIn = typeof window !== "undefined" && Boolean(readToken(ACCESS_KEY));
          ...
          {loyaltyQ.data?.data?.points && loyaltyQ.data.data.points >= 500 ? (
            <div style={{ marginTop: 12, padding: 12, background: "#F7F7FB", borderRadius: 10 }}>
              ...
              <input
                ...
                min={0}
                max={Math.min(
                  loyaltyQ.data.data.points,
                  Math.floor((baseCost - promoDiscount) * 5),
                )}
                step={100}
                value={loyaltyPts}
